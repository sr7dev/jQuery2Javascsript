...
document.addEventListener( 'DOMContentLoaded', function( event ) {
	var sliderContainer = document.querySelector( '.et_pb_slide .et_pb_container.clearfix' );
...
document.addEventListener( 'DOMContentLoaded', function( event ) {
   ...
   subTabs = document.querySelectorAll('.et_pb_tabs .et_pb_tabs_controls > [class^="et_pb_tab_"]');
	...
	for ( i = 0, subTabCount = subTabs.length ; i < subTabCount ; i ++ )
	{
		subTabs[i].addEventListener( 'mouseenter', function(e) {
			...
			if (this.classList)
			  ...
			  isActivate = new RegExp('(^| )' + 'et_pb_tab_active' + '( |$)', 'gi').test(this.et_pb_tab_active);
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  window.addEventListener('scroll', function() {
      var st = (document.documentElement || document.body.parentNode || document.body).scrollTop;

      if (st > lastScrollTop) {
          document.querySelector('.TopSection').classList.add('hide');
          document.querySelector('h2.et_pb_slide_title').classList.add('top');
          document.querySelector('.et-pb-controllers').classList.add('top-controllers');
          document.querySelector('.logo_container img#logo').classList.add('non-text');
          document.body.classList.add('hero-scrolling');
      } else {
          document.querySelector('.TopSection').classList.remove('hide');
          document.querySelector('h2.et_pb_slide_title').classList.remove('top');
          document.querySelector('.et-pb-controllers').classList.remove('top-controllers');
          document.querySelector('.logo_container img#logo').classList.remove('non-text');
...
document.addEventListener('DOMContentLoaded', function() {
   ...
   var nooffer_section_element = document.querySelector('.et_pb_tab_5 .pbe-li-sc .et_pb_section');
   ...
   function set_offer_content(target_tab, offer_type) {
      var target_element = document.querySelectorAll('.TabSection .leadresult' + offer_type);
      var source_element = document.querySelector('.LEADAPIRESULT .leadresult'+ offer_type);
      
      if (source_element !== null) {
         for (i = 0, nCount = target_element.length ; i < nCount ; i ++)
            target_element[i].innerHTML = source_element.innerHTML;
      } else {
         var dental_plan_section_element = document.querySelectorAll('.TabSection' + offer_type);
